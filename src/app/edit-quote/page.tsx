...
async function loadProductsFromJson(): Promise<Product[]> {
  try {
    ...
    return data.products as Product[]; // Access the products array
  } catch (error: any) {
...
export default function EditQuotePage() {
      ...
      if (filename) {
        try {
          ...
          if (fileContent) {
            ...
            setQuoteNumber(quoteData.quoteNumber || null);
            console.log('quoteData', quoteData);
          } else {
            toast({
              variant: 'destructive',
              title: 'Error loading quote',
              description: 'Could not read quote file.',
            });
          }
        } catch (error) {
  ...
  const onSubmit = async (values: Quote) => {
    ...
    const quoteData = {
      ...
      products: products,
      total: total,
      quoteNumber: quoteNumber, // Use existing quote number
    };
    ...
    <div className="container py-10">
      <Card>
        <CardHeader>
          ...
          />
          <h1 className="print:block hidden font-bold">Quote</h1>
          <div className="print:hidden">
            <CardTitle className="no-print">Edit Quote</CardTitle>
            <CardDescription className="no-print">
              Edit the details for your quote.
            </CardDescription>
          </div>
        </CardHeader>
                ...
                <Button
                  ...
                  onClick={() => {
                    const product = availableProducts.find(
                      p => p.description === selectedProduct
                    );
                    if (product) {
                      addProduct(product);
                    } else {
